...
export default function RFQsManagement() {
  const [statusFilter, setStatusFilter] = useState("ALL");
  const [currentPage, setCurrentPage] = useState(1);

  const { data, isLoading } = useQuery({
    queryKey: ["admin-rfqs", statusFilter, currentPage],
    queryFn: async () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-white mb-1">RFQ Management</h1>
          <p className="text-slate-400 font-semibold">
            {pagination.total} total requests for quotation
          </p>
        </div>
        ...
        <div className="rounded-2xl bg-slate-900/50 backdrop-blur border border-slate-800 overflow-hidden">
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-slate-800/50">
                {rfqs.map((rfq: any) => {
                  ...
                  return (
                    <tr key={rfq._id} className="hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div>
                          <p className="font-bold text-white">{rfq.productId?.name || "N/A"}</p>
                          <p className="text-xs text-slate-400 font-semibold">
                            {rfq.quantity} units @ ${rfq.targetUnitPrice}
                          </p>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <span className={cn(
                          ...
                          rfq.status === "CLOSED" && "bg-slate-500/10 text-slate-500"
                        )}>
                          {rfq.status}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <Link
                          ...
                          className="p-2 text-blue-400 hover:bg-blue-500/10 rounded-lg transition-colors inline-flex items-center gap-2"
                        >
                          <Eye size={18} />
                        </Link>
                      </td>
          ...
          <div className="px-6 py-4 bg-slate-800/30 flex items-center justify-between">
            <p className="text-sm text-slate-400 font-semibold">
              Showing {((currentPage - 1) * 15) + 1} to {Math.min(currentPage * 15, pagination.total)} of {pagination.total} RFQs
            </p>
            <div className="flex items-center gap-2">
              <button
