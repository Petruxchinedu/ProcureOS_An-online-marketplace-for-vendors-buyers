...
export default function VendorDashboard() {
  ...
  const { data: rfqs, isLoading: rfqsLoading } = useQuery({
    queryKey: ["vendor-rfqs-dashboard"],
    queryFn: async () => {
  ...
  const { data: products, isLoading: productsLoading } = useQuery({
    queryKey: ["vendor-products-dashboard"],
    queryFn: async () => {
  ...
  const isLoading = rfqsLoading || productsLoading;
  ...
  const rejectedRFQs = rfqs?.filter((r: any) => r.status === "REJECTED").length || 0;
    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="rounded-xl sm:rounded-2xl bg-slate-900/50 backdrop-blur border border-slate-800 p-4 sm:p-6">
        <div className="flex items-center justify-between mb-4 sm:mb-6">
          <h2 className="text-lg sm:text-xl font-black text-white">Recent RFQs</h2>
          <Link
            href="/vendor/rfq"
        ...
        {rfqs && rfqs.length > 0 ? (
          <div className="space-y-3">
            {rfqs.slice(0, 5).map((rfq: any) => (
              <Link
                key={rfq._id}
                href={`/vendor/rfq`}
                className="block p-3 sm:p-4 rounded-lg sm:rounded-xl bg-slate-800/30 hover:bg-slate-800/50 transition-colors"
              >
          ...
          <div className="text-center py-12">
            ...
            <p className="text-slate-400 font-semibold">No RFQs yet</p>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        >
          ...
          <h3 className="text-base sm:text-lg font-black text-white mb-1">Manage RFQs</h3>
        ...
        >
          <Package className="w-8 h-8 sm:w-10 sm:h-10 text-purple-400 mb-3 sm:mb-4 group-hover:scale-110 transition-transform" />
          <h3 className="text-base sm:text-lg font-black text-white mb-1">My Products</h3>
          <p className="text-xs sm:text-sm text-slate-400 font-semibold">
        ...
        >
          <CheckCircle className="w-8 h-8 sm:w-10 sm:h-10 text-emerald-400 mb-3 sm:mb-4 group-hover:scale-110 transition-transform" />
